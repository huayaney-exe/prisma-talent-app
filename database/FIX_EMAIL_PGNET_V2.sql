-- Fix send_email_via_resend - correct pg_net.http_post response structure
-- Error: column "status" does not exist
-- Issue: net.http_post returns a composite type, need to access fields correctly
-- Solution: Use proper field access for net.http_response_result
...
DECLARE
  ...
  email_body TEXT;
  resend_api_key TEXT;
  max_wait INTEGER := 30; -- Wait up to 30 seconds for response
...
BEGIN
  ...
  -- Queue HTTP request (pg_net is async)
  ...
  RAISE NOTICE 'HTTP request queued: ID=%', http_request_id;
  ...
  LOOP
    ...
    IF wait_count >= max_wait THEN
      ...
      SET
        ...
        last_error = 'HTTP request timeout after 30 seconds',
        next_retry_at = NOW() + INTERVAL '5 minutes',
        status = 'pending'
      WHERE id = email_id;

      RAISE WARNING '⏱️ Email send timeout for %', email_id;
      RETURN;
    END IF;
  END LOOP;

  RAISE NOTICE 'HTTP Response received: status_code=%, content=%',
  ...
  IF http_response.status_code = 200 THEN
    ...
    SET
      sent_at = NOW(),
      status = 'sent',
      resend_email_id = (http_response.content::jsonb->>'id')
    WHERE id = email_id;
  ...
  ELSE
    ...
    SET
      retry_count = COALESCE(retry_count, 0) + 1,
      last_error = format('HTTP %s: %s', http_response.status_code, http_response.content),
      next_retry_at = NOW() + INTERVAL '5 minutes',
      status = CASE
...
SELECT 'Function updated with correct pg_net async handling' as status;
