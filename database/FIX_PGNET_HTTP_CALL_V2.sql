-- ============================================================================
-- FIX V2: Update invite_client to handle pg_net HTTP response correctly
-- ============================================================================
-- Issue: pg_net 0.19.5 has different response structure
-- Fix: Use async HTTP request with proper response structure handling
  ...
  WHILE wait_count < max_wait LOOP
    ...
    IF FOUND THEN
      ...
      ELSE
        ...
        RETURN jsonb_build_object(
          'success', false,
          'error', format('Invitation failed: HTTP %s - %s', response_status, response_content)
        );
      END IF;
    END IF;
