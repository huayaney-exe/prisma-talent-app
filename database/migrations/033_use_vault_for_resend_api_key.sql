-- Migration 033: Update send_email_via_resend() to use vault for API key
-- Date: 2025-10-23
-- Purpose: Fix "permission denied to set parameter" error
-- Context: Supabase doesn't allow ALTER DATABASE SET for custom parameters
-- Solution: Use vault.decrypted_secrets instead of current_setting()

-- ============================================================================
-- PROBLEM ANALYSIS
-- ============================================================================
-- Original function used: current_setting('app.resend_api_key', true)
-- Attempted: ALTER DATABASE postgres SET app.resend_api_key = '...'
-- Result: ERROR 42501: permission denied to set parameter
-- Reason: Supabase restricts ALTER DATABASE permissions for security

-- ============================================================================
-- SOLUTION: USE VAULT FOR SECRET STORAGE
-- ============================================================================
-- Step 1: Store API key in vault (run manually first):
--   SELECT vault.create_secret('re_MJrfYpFi_5BTo4wyogiZsUSr28u75jWRa', 'resend_api_key');
--
-- Step 2: Update function to retrieve from vault (this migration)

-- ============================================================================
-- UPDATE FUNCTION TO USE VAULT
-- ============================================================================

CREATE OR REPLACE FUNCTION public.send_email_via_resend(email_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  ...
  resend_api_key TEXT;
BEGIN
  -- Get API key from vault (CHANGED FROM current_setting)
...
DO $$
BEGIN
  -- Verify function exists and uses vault
  IF EXISTS (
    SELECT 1 FROM pg_proc
    WHERE proname = 'send_email_via_resend'
  ) THEN
    RAISE NOTICE '✅ Function send_email_via_resend() updated successfully';
    RAISE NOTICE '🔑 Function now retrieves API key from vault.decrypted_secrets';
    RAISE NOTICE '📧 Email sending should now work with vault-stored API key';
    RAISE NOTICE '⚠️  Ensure vault secret exists: SELECT * FROM vault.secrets WHERE name = ''resend_api_key''';
  ELSE
    RAISE EXCEPTION '❌ Function send_email_via_resend() not found';
  END IF;
END $$;
